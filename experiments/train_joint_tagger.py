...
if __name__ == "__main__":
    ...
    argparser.add_argument('--pre_trained_model_resize', dest='pre_trained_model_resize',
                           action='store_true')
    ...
    if train:
        ...
        if args.distant_file is not None:
            distant_set = JointRelatedWorkAnnotationDataset(args.distant_file, tokenizer, MAX_SENT_LEN = args.MAX_SENT_LEN, dummy_discourse=False, dummy_span=False, dummy_citation=False) #####
    ...
    if not (args.pre_trained_model is not None and args.pre_trained_model_resize):
    ...
    if test:
        ...
        dev_discourse_score, dev_citation_score, dev_span_score = evaluation(model, dev_set)
        print(
            f'Test discourse f1 p r: %.4f, %.4f, %.4f' % dev_discourse_score)
        print(
            f'Test citation f1 p r: %.4f, %.4f, %.4f' % dev_citation_score)
        print(
            f'Test span f1 p r: %.4f, %.4f, %.4f' % dev_span_score)
