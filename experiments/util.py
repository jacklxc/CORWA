...
def realign_samples(dataset, paragraph_ids):
    if len(dataset.samples) == len(paragraph_ids):
        included_sample_idx = [i for i in range(len(dataset.samples))]
    else:
        actual_idx = 0
        included_sample_idx = []
        for i, sample in enumerate(dataset.samples):
            if paragraph_ids[actual_idx] == sample["id"]:
                included_sample_idx.append(i)
                actual_idx += 1
    return included_sample_idx

def citation_prediction_to_annotation_paragraph(dataset, citation_predictions,
                                                tokenizer, sample_indices = None):
    ...
    if sample_indices is None:
        sample_indices = [i for i in range(len(dataset.samples))]
    all_annotations = []
    for sample_idx, citation_prediction in zip(sample_indices,
...
def paragraph2doc_annotation(dataset, all_annotations, tokenizer, sample_indices = None):
    text = ""
    paper_ids = []
    all_texts = []
    annotation_by_doc = []
    if sample_indices is None:
        sample_indices = [i for i in range(len(dataset.samples))]
    for sample_idx, annotations in zip(sample_indices, all_annotations):
        sample = dataset.samples[sample_idx]
...
def span_prediction_to_annotation_paragraph(dataset, span_predictions,
    ...
    if sample_indices is None:
        sample_indices = [i for i in range(len(dataset.samples))]
    for sample_idx, span_prediction in zip(sample_indices, span_predictions):
        sample = dataset.samples[sample_idx]
        original_paragraph = sample["paragraph"].replace(tokenizer.sep_token,
                                                         "[BOS]")
        offset_mapping = \
...
def discourse_prediction_to_annotation_paragraph(dataset, discourse_predictions,
    ...
    if sample_indices is None:
        sample_indices = [i for i in range(len(dataset.samples))]
    for sample_idx, discourse_prediction in zip(sample_indices,
